...
module.exports = {
    ...
    deleteSubCategory: function (req,res){
        subCategoryModel.findByIdAndRemove ( {_id: req.params.id}, (err, SubCategory) => { 
            if (err) {
                res.status(500), json({
                    msg: "erreur",
                    status: 500,
    ...
    updateSubCategory: function (req, res) {
        subCategoryModel.findByIdAndUpdate( {_id: req.params.id} , req.body ).exec(function (err, SubCategorys) {
            if (err) {
                res.status(500), json({
                    msg: "erreur",
                    status: 500,
    ...
    getSubCategory: function (req, res) {
        subCategoryModel.findById( {_id: req.params.id} ).populate('cat_id').exec(function (err, SubCategorys) {
            if (err) {
                res.status(500), json({
                    msg: "erreur",
                    status: 500,
    ...
    getAllSubCategorys: function (req, res) {
        subCategoryModel.find({}).populate('cat_id').exec(function (err, SubCategorys) {
            if (err) {
                res.status(500), json({
                    msg: "erreur",
                    status: 500,
